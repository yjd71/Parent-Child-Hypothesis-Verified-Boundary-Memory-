...
def build_pred_boundary(
    ...
    prob = as_4d_mask(prob, "prob").float().clamp_(0.0, 1.0)
    p_bin = (prob >= 0.5).to(dtype=prob.dtype)

    b_morph = (dilate(p_bin, kernel=kernel) - erode(p_bin, kernel=kernel)).clamp_(0.0, 1.0)
    b_unc = (4.0 * prob * (1.0 - prob)).clamp_(0.0, 1.0)
    ...
    b_query = (b_query / denom).clamp_(0.0, 1.0)
