...
class statistics():
    ...
    def step(self, gts, preds):
        if len(gts.shape) == 4:
            gts = gts.squeeze(1)
        if len(preds.shape) == 4:
            preds = preds.squeeze(1)
        gt_arys = gts.to('cpu').numpy()
        pred_arys = preds.to('cpu').numpy()
        sms = list()
        maes = list()
        # wfms = list()
        for gt_ary, pred_ary in zip(gt_arys, pred_arys):
            pred, gt = _prepare_data(pred=pred_ary, gt=gt_ary)
            sm = self.SM.cal_sm(pred = pred, gt = gt)
            mae = self.MAE.cal_mae(pred = pred, gt = gt)
            # wfm = self.WFM.cal_wfm(pred = pred_ary, gt = gt_ary)
            sms.append(sm)
            maes.append(mae)
            # wfms.append(wfms)
        sms = torch.tensor(sms)
        maes = torch.tensor(maes)
        # wfms = torch.cat(wfms, dim=-1)

        return sms, maes 


def calculate(gt_paths, pred_paths, metrics=['S', 'MAE', 'E', 'F', 'WF'], verbose=False):
    # define measures
    if 'E' in metrics:
        EM = Emeasure()
    if 'S' in metrics:
