...
class BasicDecBlk(nn.Module):
    def __init__(self, config, in_channels=64, out_channels=64, inter_channels=64):
        super(BasicDecBlk, self).__init__()
        inter_channels = 64
        self.conv_in = nn.Conv2d(in_channels, inter_channels, 3, 1, padding=1)
        self.relu_in = nn.ReLU(inplace=True)
        self.dec_att = ASPPDeformable(config, in_channels=inter_channels)
...
class ResBlk(nn.Module):
    def __init__(self, config, in_channels=64, out_channels=None, inter_channels=64):
