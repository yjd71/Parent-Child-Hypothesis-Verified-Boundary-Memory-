...
def _reference_rsbl_structure_loss(logits: torch.Tensor, hard_target: torch.Tensor) -> torch.Tensor:
...
def test_soft_teacher_loss_keeps_fractional_targets_and_total_formula():
    ...
    config = _config(lambda_u=0.25, hard_weight=2.0, use_soft_teacher_weighted_iou=False)
    ...
    expected_total = 0.25 * expected_soft + 2.0 * expected_hard
...
def test_disabled_hard_teacher_loss_skips_target_and_loss_computation():
    ...
    try:
        ...
        total, log = compute_pc_hbm_unlabeled_loss(
            ...
            pseudo,
            confidence,
            _config(use_hard_teacher_loss=False),
        )
    finally:
